...
def test_youtube_channel_stats():
    ...
    for channel_identifier in test_channels[:1]:
        ...
        if "subscriber_count_hidden" in stats and stats["subscriber_count_hidden"]:
            ...
            assert (
                stats.get("subscriber_count") is None
            ), "Hidden subscriber count should be None"
        else:
            assert (
                "subscriber_count" in stats
            ), "subscriber_count is required when not hidden"
            assert isinstance(
                stats["subscriber_count"], int
            ), "subscriber_count should be an integer"
...
def test_batch_fetch_with_channel_ids():
    ...
    results = account.get_youtube_channels_batch(
        channel_identifiers=channel_ids, api_key=youtube_api_key, artist_ids=None
    )
...
def test_batch_fetch_with_mixed_formats():
    ...
    results = account.get_youtube_channels_batch(
        channel_identifiers=mixed_inputs, api_key=youtube_api_key, artist_ids=None
    )
...
def test_batch_large_scale():
    ...
    results = account.get_youtube_channels_batch(
        channel_identifiers=channel_ids, api_key=youtube_api_key, artist_ids=artist_ids
    )
    ...
    print(
        f"Artist ID mapping working: {results[0]['artist_id']} -> {results[0]['channel_title']}"
    )
...
def test_search_youtube_channels():
    ...
    search_results = account.search_youtube_channels(
        query="technology reviews", api_key=youtube_api_key, max_results=3
    )
...
def test_error_handling():
    ...
    with pytest.raises(Exception) as exc_info:
        stats = account.get_youtube_channel_stats(
            channel_identifier="@mkbhd", api_key="invalid_api_key_12345", video_count=5
        )
    error_message = str(exc_info.value)
    print(f"Invalid API key error: {error_message}")
    assert any(
        x in error_message.lower() for x in ["api", "401", "403", "400", "bad request"]
    ...
    assert any(
        x in error_message.lower() for x in ["not found", "404", "channel not found"]
...
def test_url_parsing():
    ...
    results = account.get_youtube_channels_batch(
        channel_identifiers=url_formats, api_key=youtube_api_key, artist_ids=None
    )
