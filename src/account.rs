    ...
    match id_type {
        ...
        IdentifierType::Handle => {
            ...
            if resp.status().is_success() {
                let data: YouTubeChannelResponse = resp
                    .json()
            ...
            let search_data: YouTubeSearchResponse = search_resp.json().map_err(|e| {
                PyValueError::new_err(format!("Failed to parse search results: {}", e))
            })?;

            let first_channel = search_data
                .items
                .into_iter()
                .next()
            ...
            if let Some(channel_id) = &first_channel.id.channel_id {
                ...
                return Err(PyValueError::new_err(
                    "Channel ID not found in search result",
                ));
            }
        }
...
pub fn get_youtube_channel_stats(
    ...
    if videos_to_fetch > 0 && channel.content_details.is_some() {
        ...
        if let Ok(videos_response) = client
            .get(&videos_url)
            .header("Accept", "application/json")
            .send()
        {
            if videos_response.status().is_success() {
                if let Ok(videos_data) = videos_response.json::<YouTubeVideoListResponse>() {
                    // Get video IDs
                    let video_ids: Vec<String> = videos_data
                        .items
                        .iter()
                    ...
                    if !video_ids.is_empty() {
                        ...
                        if let Ok(stats_response) = client
                            .get(&video_stats_url)
                            .header("Accept", "application/json")
                            .send()
                        {
                            if stats_response.status().is_success() {
                                if let Ok(stats_data) =
                                    stats_response.json::<YouTubeVideoListResponse>()
                                {
    ...
    Python::with_gil(|py| {
        ...
        // Recent videos
        let py_videos = PyList::new(
            py,
            recent_videos.iter().map(|video| {
                ...
                video_dict
                    .set_item("published_at", &video.snippet.published_at)
                    .unwrap();
        ...
        let total_recent_views: u64 = recent_videos
            .iter()
        ...
        let total_recent_likes: u64 = recent_videos
            .iter()
            .filter_map(|v| v.statistics.as_ref())
        ...
        let total_recent_comments: u64 = recent_videos
            .iter()
            .filter_map(|v| v.statistics.as_ref())
...
pub fn search_youtube_channels(
    ...
    let search_results: YouTubeSearchResponse = response
        .json()
        .map_err(|e| PyValueError::new_err(format!("Failed to parse search results: {}", e)))?;

    Python::with_gil(|py| {
        let py_dicts: Vec<Py<PyDict>> = search_results
            .items
            .iter()
            .filter_map(|item| {
                if let Some(channel_id) = &item.id.channel_id {
                    ...
                    channel_dict
                        .set_item("description", &item.snippet.description)
                        .unwrap();
                    channel_dict
                        .set_item(
                            "channel_url",
